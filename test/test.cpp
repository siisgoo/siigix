...
on_connect(TCP::Server::Client& cli)
{
    ...
    std::cout << "new client connected. ip: " << cli.getIP() << std::endl;
}

void
on_disconnect(TCP::Server::Client& cli)
{
    std::cout << "client disconnected. ip: " << cli.getIP() << std::endl;
}

void
on_data_recive(const std::string& data, TCP::Server::Client& cli)
{
    std::cout << "data recved: " << data << std::endl;
}

int
main(int argc, char** argv)
{
    port_t port = (argc > 1) ? atoi(argv[1]) : 8080;
