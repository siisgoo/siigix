...
{
    namespace TCP {
        Server::Server(port_t port, int max_conn, int threads) :
        ...
        Server::Server(data_hndl_fn_t d_hndl_fn,
                ...
                port_t port, int max_conn, int threads) :
            _socket(port, { sock_opt(SOL_SOCKET, SO_REUSEADDR, USE_OPT_FLAG, sizeof(USE_OPT_FLAG)) }, max_conn),
        ...
        Server::waitDataLoop()
        {
            ...
            for (auto it = _clients.begin(), end = _clients.end(); it != end; ++it)
            {
                ...
                if (client->_status == ClientStatus::connected) /* client reciving data */
                {
