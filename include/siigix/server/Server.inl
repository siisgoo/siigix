...
Server<ProtocolC>::Server(std::string host, port_t port, bool useip6, bool useudp, int max_conn, int threads) :
    Server(
            [](const std::string& data, Client<ProtocolC>&) {},
            [](Client<ProtocolC>&) {},
            [](Client<ProtocolC>&) {},
...
Server<ProtocolC>::Server(
    ...
    _socket(host, port,
            {
                .ai_family = (useip6) ? AF_INET6 : AF_INET,
                .ai_socktype = (useudp) ? SOCK_DGRAM : SOCK_STREAM,
                .ai_protocol = 0
            },
            {
                sock_opt(SOL_SOCKET, SO_REUSEADDR, USE_OPT_FLAG, sizeof(USE_OPT_FLAG))
            },
            max_conn),
